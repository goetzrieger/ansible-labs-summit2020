...
|Control/Bastion Host, Gitea Repo|bastion.${GUID}.sandbox${SBUID}.opentlc.com|bastion.${GUID}.internal
...
// Your lab environment includes Gitea, a Git-service that comes with a web ui and much more. Gitea runs on `bastion.${GUID}.com` and can be accessed via HTTP. Go and have a look around by accessing:

// *\http://bastion.${GUID}.sandbox${SBUID}.opentlc.com/gitea*
...
// WARNING: Remember you can use `bastion.${GUID}.com` as hostname in *SCM URL* because it resolves inside the environment. For accessing the Gitea web UI in your browser you need to use `\http://bastion.${GUID}.sandbox${SBUID}.opentlc.com/gitea`.
...
----
awx -k job_template create -n "Compliance STIG packages" \
...
----
awx -k job_template create -n "Compliance STIG config" \
                    --job_type run -i "Example Inventory" \
                    --project "Apache" \
...
----
awx -k job_template create -n "Compliance CIS" \
...
[ec2-user@bastion ~]$ git clone git@bastion.${GUID}.com:/git/structured-content.git
...
[ec2-user@bastion-dff2 structured-content]$ git push
----
...
----
[ec2-user@bastion structured-content]$ ansible-playbook -i staging site.yml -b
----

Watch how the changes are done to the target machines. Afterwards, execute the Playbook against the production stage:

----
[ec2-user@bastion structured-content]$ ansible-playbook -i production site.yml -b
----
...
----
awx -k project create -n "Structured Content Repository" \
                    --organization Default \
                    --scm_type git \
                    --scm_url http://bastion.${GUID}.com/gitea/git/structured-content.git \
                    --scm_clean 1 \
...
----
awx -k job_template create -n "Structured Content Execution" \
...
- src: http://bastion.${GUID}.com/gitea/git/external-role.git
  version: master
...
In this example, we will include a role which ships a simple `index.html` file as template and reloads the apache web server. The role is already shared in Gitea at *\http://bastion.${GUID}.com/gitea/git/shared-apache-role*.
...
- src: http://bastion.${GUID}.com/gitea/git/shared-apache-role.git
  scm: git
  version: master
----
