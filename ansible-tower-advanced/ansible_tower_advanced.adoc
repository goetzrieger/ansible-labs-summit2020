...
----
yum-config-manager --add-repo https://releases.ansible.com/ansible-tower/cli/ansible-awx-cli-el7.repo
...
----
[root@control ~]# awx-cli inventory list
----
...
----
[root@control ~]# awx-cli host create --name "host1.example.com" --inventory "Example Inventory"
[root@control ~]# awx-cli host create --name "host2.example.com" --inventory "Example Inventory"
----
...
TIP: If you run the script a second time, you will see that *awx-cli* is idempotent, so it's fine that you run the *awx-cli* commands already.
...
Now with the SCM credentials configured, the next step is to add a project to import the playbooks. Add the appropriate *awx-cli* line to the script *setup-tower.sh*:
...
----
[root@control ~]# awx-cli instance_group list
== ===== ======== ================= 
...
As mentioned the repository contains three Playbooks to enforce different compliance requirements. We again create these three templates via `awx-cli`:
