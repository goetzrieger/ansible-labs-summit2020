...
```bash
[{{< param "control_prompt" >}} ansible-files]$ scp 11.22.33.44:/etc/httpd/conf/httpd.conf ~/ansible-files/
httpd.conf
```
...
[{{< param "control_prompt" >}} ansible-files]$ curl http://22.33.44.55:8080
<body>
...
```bash
[{{< param "control_prompt" >}} ansible-files]$ ansible node1 -m command -a "id dev_user"
node1 | CHANGED | rc=0 >>
