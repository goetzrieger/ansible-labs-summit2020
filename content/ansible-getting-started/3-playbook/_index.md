...
```bash
[student<N>@ansible-1 ~]$ mkdir ansible-files
[student<N>@ansible-1 ~]$ cd ansible-files/
```
...
```bash
[student<N>@ansible-1 ansible-files]$ ansible-playbook --syntax-check apache.yml
```

Now you should be ready to run your Playbook:

```bash
[student<N>@ansible-1 ansible-files]$ ansible-playbook apache.yml
```
...
```bash
[student<N>@ansible-1 ansible-files]$ grep node1 ~/lab_inventory/hosts
node1 ansible_host=11.22.33.44
[student<N>@ansible-1 ansible-files]$ ssh 11.22.33.44
...
```bash
[student<N>@ansible-1 ansible-files]$ ansible node1 -m command -a 'rpm -qi httpd'
```
...
```bash
[student<N>@ansible-1 ansible-files]$ ansible localhost -m uri -a "url=http://<IP>"
```
